...
const crashGenerateGame = () => {
    return new Promise(async(resolve, reject) => {
        try {
            ...
            // Auto-create a seed if none exist
            if(seedDatabase === null) {
                const seedServer = crypto.randomBytes(32).toString('hex');
                const hash = crypto.createHash('sha256').update(seedServer).digest('hex');
                seedDatabase = await CrashSeed.create({ seedServer, hash, state: 'created' });
                seedDatabase = seedDatabase.toObject();
            }
